...
TEST ( HttpParserTest, FormPost ) {
    buffer_t _buffer;
    size_t _size = get_file ( TESTFILES + std::string ( "/raw/request/form_post.dump" ), &_buffer );
    HttpParser _parser;
    HttpParser::RequestParserState _state;
    http::Request _request;

    size_t _position = _parser.parse_request_status_line ( _state, _request, _buffer, 0, _size );
    EXPECT_EQ ( _position, 23U );

    EXPECT_EQ ( "POST", _request.method() );
    EXPECT_EQ ( "/config", _request.uri() );
    EXPECT_EQ ( "HTTP", _request.protocol() );
    EXPECT_EQ ( 1, _request.version_major() );
    EXPECT_EQ ( 1, _request.version_minor() );

    _position = _parser.parse_parameter ( _state, _request, _buffer, _position, _size );
    EXPECT_EQ ( _position, 452U );

    EXPECT_EQ ( 10U, _request.parameter_map().size() );
    EXPECT_EQ ( "192.168.0.1:9010", _request.parameter ( "Host" ) );
    EXPECT_EQ ( "keep-alive", _request.parameter ( "Connection" ) );
    EXPECT_EQ ( "1", _request.parameter ( "Upgrade-Insecure-Requests" ) );
    EXPECT_EQ ( "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:61.0) Gecko/20100101 Firefox/61.0", _request.parameter ( "User-Agent" ) );
    EXPECT_EQ ( "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8", _request.parameter ( "Accept" ) );
    EXPECT_EQ ( "gzip, deflate", _request.parameter ( "Accept-Encoding" ) );
    EXPECT_EQ ( "de,en-US;q=0.7,en;q=0.3", _request.parameter ( "Accept-Language" ) );
    EXPECT_EQ ( "http://192.168.0.1:9010/config", _request.parameter ( "Referer" ) );
    EXPECT_EQ ( "69", _request.parameter ( "Content-Length" ) );
    EXPECT_EQ ( "application/x-www-form-urlencoded", _request.parameter ( "Content-Type" ) );

    _position = _parser.parse_body_form_data ( _state, _request, _buffer, _position, _size );
    EXPECT_EQ ( _position, _size );

    EXPECT_EQ ( 2U, _request.attribute_map().size() );
    EXPECT_EQ ( "Squawk Media Server (Test)", _request.attribute ( "name" ) );
    EXPECT_EQ ( "192.168.0.1", _request.attribute ( "upnp%3Alisten_address" ) );
}
...
TEST ( HttpParserTest, ParseFormPostRequest ) {
    ...
    EXPECT_EQ ( _size - std::stoul ( _request.parameter ( http::header::CONTENT_LENGTH ) ), _position );
...
TEST ( HttpParserTest, ParseChunkedWithBody ) {
    ...
    EXPECT_EQ ( _size - std::stoul ( _request.parameter ( http::header::CONTENT_LENGTH ) ), _res_position );
...
TEST ( HttpParserTest, ParseChunkedWithNewBody ) {
    ...
    size_t _res_position = 0;
    _res_position = _parser.parse_request ( _request, _buffer, _position, _position + 4 );
    _res_position = _parser.parse_request ( _request, _buffer, _position+4, _size );
